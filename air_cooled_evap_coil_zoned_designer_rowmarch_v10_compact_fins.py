...
def airside_compact_htc_dp(mdot_air, face_W, face_H, depth, fin_pitch, tf,
    ...
    if fin_type != "Wavy + Louvers":
        # Safety: unknown fin type, fall back to wavy
        fin_type = "Wavy (no louvers)"

    # Require explicit louver geometry. Without it, using a louver correlation is usually worse than using
    # a plain/wavy fin model, because the Reynolds number and characteristic lengths differ.
    if fin_type == "Wavy + Louvers":
        if (louver_pitch_m is None) or (louver_length_m is None) or (louver_pitch_m <= 0.0) or (louver_length_m <= 0.0):
            # Fall back to wavy model and flag the reason in meta.
            if Re_Dh < 2300:
                Nu = 7.54
            ...
            dp_minor = (0.5+1.0)*(rho*V*V/2.0)
            return h, (dp_core+dp_minor), {"model":"duct+wavy","Re":Re_Dh,"Dh":Dh,"A_min":A_min,"sigma":sigma,"V":V,
                                           "note":"louver_selected_but_geometry_missing"}
    Nr_est = max(int(round(depth/0.022)), 1)
    Lp_row_m = float(louver_pitch_m) if (fin_type == "Wavy + Louvers") else max((depth/max(Nr_est,1))/max(louver_cuts_per_row,1), 1e-4)
    Lp = Lp_row_m*1000.0
    Fp = s_f*1000.0
    delta = tf*1000.0
    Fh = face_H*1000.0
    Fd = depth*1000.0
    Lh = max((louver_length_m*1000.0) if (fin_type == "Wavy + Louvers" and louver_length_m is not None) else Fp, 0.1)
    Tp = max(Fp, 0.1)
...
def liquid_line_pressure_budget(
    fluid: str,
    mdot_ref: float,
    T_liq_C: float,
    P_high_bar_abs: float,
    Tsat_evap_C: float,
    dP_cond_liq_kPa: float,
    L_m: float,
    ID_mm: float,
    rough_m: float,
    K_sum: float,
    dz_m: float,
    dP_components_kPa: float = 0.0,
):
    """Compute TXV inlet pressure from high-side pressure and liquid line losses.

    Returns dict with pressure budget terms (kPa), TXV inlet pressure (bar abs),
    basic hydraulics (v, Re), and available subcooling margin at TXV inlet.

    Assumptions:
      - Single-phase liquid in the line (no flashing). We provide a subcooling margin check.
      - Liquid line temperature is T_liq_C at TXV inlet (no heat gain modeled).
      - Friction drop via Darcy–Weisbach; f from Swamee–Jain.
      - Minor losses via ΣK (user supplied).
      - Elevation term uses +dz as TXV above condenser (pressure drop).
    """
    if not HAS_CP:
        raise RuntimeError("CoolProp not available; cannot compute liquid-line pressure budget.")
    D = max(ID_mm, 0.1) / 1000.0
    A = math.pi * D * D / 4.0

    # Use high-side pressure as first estimate for properties
    P_prop_Pa = max(P_high_bar_abs, 0.5) * 1e5
    T_prop_K = T_liq_C + 273.15
    rho_l = float(PropsSI("D", "T", T_prop_K, "P", P_prop_Pa, fluid))  # kg/m3
    mu_l = float(PropsSI("V", "T", T_prop_K, "P", P_prop_Pa, fluid))  # Pa·s

    v = mdot_ref / max(rho_l * A, 1e-12)  # m/s
    Re = rho_l * v * D / max(mu_l, 1e-12)
    ...
    dp_fric_Pa = f * (L_m / max(D, 1e-12)) * (rho_l * v * v / 2.0)
    dp_min_Pa = max(K_sum, 0.0) * (rho_l * v * v / 2.0)
    dp_elev_Pa = rho_l * 9.80665 * dz_m  # +dz -> drop

    # Total drops (kPa)
    dp_total_kPa = (
        max(dP_cond_liq_kPa, 0.0)
        + dp_fric_Pa / 1000.0
        + dp_min_Pa / 1000.0
        + max(dP_components_kPa, 0.0)
        + dp_elev_Pa / 1000.0
    )

    P_txv_bar_abs = P_high_bar_abs - dp_total_kPa / 100.0  # (kPa -> bar)
    P_txv_bar_abs = max(0.2, P_txv_bar_abs)

    # Subcooling margin at TXV inlet (to avoid flashing in line)
    try:
        Tsat_txv_C = float(PropsSI("T", "P", P_txv_bar_abs * 1e5, "Q", 0, fluid) - 273.15)
        subcool_margin_K = Tsat_txv_C - T_liq_C
    except Exception:
        Tsat_txv_C = float("nan")
        subcool_margin_K = float("nan")

    return {
        "P_high_bar_abs": P_high_bar_abs,
        "P_txv_bar_abs": P_txv_bar_abs,
        "dP_cond_liq_kPa": float(max(dP_cond_liq_kPa, 0.0)),
        "dP_fric_kPa": float(dp_fric_Pa / 1000.0),
        "dP_minors_kPa": float(dp_min_Pa / 1000.0),
        "dP_components_kPa": float(max(dP_components_kPa, 0.0)),
        "dP_elev_kPa": float(dp_elev_Pa / 1000.0),
        "dP_total_kPa": float(dp_total_kPa),
        "rho_l_kg_m3": float(rho_l),
        "mu_l_Pas": float(mu_l),
        "v_m_s": float(v),
        "Re": float(Re),
        "f": float(f),
        "Tsat_txv_C": Tsat_txv_C,
        "subcool_margin_K": subcool_margin_K,
    }
...
def simulate_evaporator(
    ...
    h_air_dry, dp_air, air_meta = airside_compact_htc_dp(mdot_air_total, face_W, face_H, geom.get('depth', Nr*St), geom['fin_pitch'], tf,
                                               ...
                                               louver_pitch_m=(louver_pitch_mm/1000.0) if fin_type=="Wavy + Louvers" else None,
                                               louver_length_m=(louver_length_mm/1000.0) if fin_type=="Wavy + Louvers" else None,
...
with c8:
    ...
    if fin_type == "Wavy + Louvers":
        ...
        louver_pitch_mm = st.number_input("Louver pitch Lp (mm)", 0.2, 10.0, 1.5, 0.1, format="%.2f")
        louver_length_mm = st.number_input("Louver length/height Lh (mm)", 0.5, 20.0, 6.0, 0.5, format="%.1f")
        st.info("Louver correlations need real louver geometry (Lp, Lh). If these are not representative, results can be very wrong.")
    else:
        louver_angle_deg = 27.0
        louver_cuts_per_row = 8
        louver_pitch_mm = 0.0
        louver_length_mm = 0.0
...
use_calc_x = st.checkbox("Compute x_in from condenser + liquid line (recommended)", value=False)

txv_budget = None
x_in_calc_mode = "manual"

if use_calc_x:
    st.caption("We will compute TXV inlet pressure from high-side pressure minus condenser+liquid-line losses, then compute flash quality x_in by isenthalpic expansion to evaporator pressure.")

    calc_mode = st.radio("TXV inlet calculation method", ["From liquid line pressure+temperature (direct)", "From condenser outlet + liquid line losses"], index=1)

    if calc_mode == "From liquid line pressure+temperature (direct)":
        P_txv_bar = st.number_input("TXV inlet pressure (bar abs)", 1.0, 60.0, 17.0, 0.1, format="%.2f")
        T_txv_C = st.number_input("TXV inlet temperature (°C)", -20.0, 80.0, 48.0, 0.1, format="%.1f")
        x_in_calc_mode = "direct_P_T"
        if HAS_CP:
            try:
                h_txv_in = PropsSI("H","T",T_txv_C+273.15,"P",P_txv_bar*1e5,fluid)  # J/kg
                P_evap = PropsSI("P","T",Tsat+273.15,"Q",0,fluid)  # Pa
                h_f = PropsSI("H","P",P_evap,"Q",0,fluid)
                h_g = PropsSI("H","P",P_evap,"Q",1,fluid)
                x_in = float((h_txv_in - h_f)/max((h_g - h_f), 1e-9))
                x_in = max(0.0, min(0.9999, x_in))
                st.success(f"Computed x_in = {x_in:.3f} (flash gas mass fraction after TXV)")
            except Exception as e:
                st.error(f"Could not compute x_in: {e}")
                x_in = st.number_input("Inlet quality after TXV (x_in)", 0.0, 0.95, 0.25, 0.01, format="%.2f")
        else:
            st.warning("CoolProp not available; cannot compute x_in automatically.")
            x_in = st.number_input("Inlet quality after TXV (x_in)", 0.0, 0.95, 0.25, 0.01, format="%.2f")

    else:
        # High-side / condenser outlet data
        P_high_bar = st.number_input("Compressor discharge / condensing pressure (bar abs)", 1.0, 60.0, 17.5, 0.1, format="%.2f")
        T_liq_out_C = st.number_input("Condenser outlet liquid temperature (°C)", -20.0, 80.0, 48.0, 0.1, format="%.1f")
        dP_cond_liq_kPa = st.number_input("Condenser liquid-side pressure drop (kPa)", 0.0, 200.0, 15.0, 1.0, format="%.1f")

        st.subheader("Liquid line")
        L_liq_m = st.number_input("Liquid line length (m)", 0.0, 200.0, 10.0, 0.5, format="%.1f")
        ID_liq_mm = st.number_input("Liquid line inner diameter (mm)", 1.0, 60.0, 10.0, 0.5, format="%.1f")

        rough_choice = st.selectbox("Pipe roughness (typical)", ["Smooth (1e-6 m)", "Copper (1.5e-6 m)", "Steel commercial (4.5e-5 m)"], index=1)
        rough_map = {"Smooth (1e-6 m)":1e-6, "Copper (1.5e-6 m)":1.5e-6, "Steel commercial (4.5e-5 m)":4.5e-5}
        eps_m = rough_map.get(rough_choice, 1.5e-6)

        st.subheader("Fittings / elevation / components")
        K_sum = st.number_input("ΣK minor loss coefficient (bends/valves/fittings)", 0.0, 200.0, 6.0, 0.5, format="%.1f")
        dz_m = st.number_input("Elevation change Δz (m) (positive if TXV above condenser)", -50.0, 50.0, 7.0, 0.5, format="%.1f")
        dP_components_kPa = st.number_input("Extra liquid-line component drop (filter-drier/solenoid/etc) (kPa)", 0.0, 200.0, 10.0, 1.0, format="%.1f")

        x_in_calc_mode = "budget"
        if HAS_CP:
            try:
                txv_budget = liquid_line_pressure_budget(
                    fluid=fluid,
                    mdot_ref=mdot_ref,
                    T_liq_C=T_liq_out_C,
                    P_high_bar_abs=P_high_bar,
                    Tsat_evap_C=Tsat,
                    dP_cond_liq_kPa=dP_cond_liq_kPa,
                    L_m=L_liq_m,
                    ID_mm=ID_liq_mm,
                    rough_m=eps_m,
                    K_sum=K_sum,
                    dz_m=dz_m,
                    dP_components_kPa=dP_components_kPa,
                )
                P_txv_bar = txv_budget["P_txv_bar_abs"]
                st.write("### TXV inlet pressure budget (kPa)")
                budget_df = pd.DataFrame([{
                    "dP_cond_liq": txv_budget["dP_cond_liq_kPa"],
                    "dP_fric": txv_budget["dP_fric_kPa"],
                    "dP_minors": txv_budget["dP_minors_kPa"],
                    "dP_components": txv_budget["dP_components_kPa"],
                    "dP_elev": txv_budget["dP_elev_kPa"],
                    "dP_total": txv_budget["dP_total_kPa"],
                }])
                st.dataframe(budget_df, use_container_width=True)
                st.info(f"P_high = {P_high_bar:.2f} bar abs → P_TXV,in ≈ {P_txv_bar:.2f} bar abs.  Subcooling margin ≈ {txv_budget['subcool_margin_K']:.2f} K (positive is good).")

                # Compute x_in via isenthalpic expansion
                h_txv_in = PropsSI("H","T",T_liq_out_C+273.15,"P",P_txv_bar*1e5,fluid)  # J/kg
                P_evap = PropsSI("P","T",Tsat+273.15,"Q",0,fluid)  # Pa
                h_f = PropsSI("H","P",P_evap,"Q",0,fluid)
                h_g = PropsSI("H","P",P_evap,"Q",1,fluid)
                x_in = float((h_txv_in - h_f)/max((h_g - h_f), 1e-9))
                x_in = max(0.0, min(0.9999, x_in))
                st.success(f"Computed x_in = {x_in:.3f} (flash gas mass fraction after TXV)")
                if (not math.isnan(txv_budget['subcool_margin_K'])) and txv_budget['subcool_margin_K'] < 0:
                    st.warning("Negative subcooling margin at TXV inlet: flashing may occur in the liquid line. Increase subcooling or reduce pressure drop.")
            except Exception as e:
                st.error(f"Could not compute TXV inlet pressure / x_in: {e}")
                x_in = st.number_input("Inlet quality after TXV (x_in)", 0.0, 0.95, 0.25, 0.01, format="%.2f")
        else:
            st.warning("CoolProp not available; cannot compute x_in automatically.")
            x_in = st.number_input("Inlet quality after TXV (x_in)", 0.0, 0.95, 0.25, 0.01, format="%.2f")
...
if run:
    try:
        ...
        # Add TXV inlet pressure budget info to report if computed
        if use_calc_x:
            inputs_dict["x_in calc mode"] = x_in_calc_mode
            if txv_budget is not None:
                inputs_dict["P_high (bar abs)"] = txv_budget.get("P_high_bar_abs")
                inputs_dict["P_TXV,in (bar abs)"] = txv_budget.get("P_txv_bar_abs")
                inputs_dict["dP_total_liquid_path (kPa)"] = txv_budget.get("dP_total_kPa")
                inputs_dict["dP_cond_liq (kPa)"] = txv_budget.get("dP_cond_liq_kPa")
                inputs_dict["dP_liq_fric (kPa)"] = txv_budget.get("dP_fric_kPa")
                inputs_dict["dP_liq_minors (kPa)"] = txv_budget.get("dP_minors_kPa")
                inputs_dict["dP_liq_components (kPa)"] = txv_budget.get("dP_components_kPa")
                inputs_dict["dP_liq_elevation (kPa)"] = txv_budget.get("dP_elev_kPa")
                inputs_dict["Subcool margin at TXV,in (K)"] = txv_budget.get("subcool_margin_K")
